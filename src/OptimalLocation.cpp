...
MyPreObjFunction::MyPreObjFunction(Manager &mgr, std::unordered_map<std::string, FF *> &FF_list, std::unordered_map<string, int> &idx_map, int totalFF, std::vector<FF *> &FFs)
    : objFunction(mgr, FF_list, idx_map, totalFF, FFs)
{
...
MyPreObjFunction::~MyPreObjFunction()
{
}
void MyPreObjFunction::getWeight(FF *cur_ff, std::vector<double> &weight)
{
    // weigt by slack
...
double MyPreObjFunction::forward()
{
 ...
 */
vector<Coor> &MyPreObjFunction::backward(int step, bool onlyNegative)
{
    // TODO: how to calculate?
