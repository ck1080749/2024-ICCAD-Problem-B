...
void Preprocess::Debank()
{
    ...
    for (auto &mbff_m : mgr.FF_Map)
    {
        ...
        for (int i = 0; i < cur_ff.getPinCount(); i++)
        {
            ...
            if (pinName[0] == 'D')
            { // Assume all ff D pin start with D
                FF *temp = new FF;
                Coor ff_coor;
                Cell *ff_cell;
...
void Preprocess::optimal_FF_location()
{
    ...
    for (auto &FF_m : FF_list)
    {
        ...
        if (1 || !curFF->getFixed())
        { // ignore fixed idea, it does improve :(
 ...
 * @attention  Does force smaller to help to choose more-legalizable celltype, since you use
...
void Preprocess::connectNet(const Net &n, std::string &driving_cell, std::string &driving_pin)
{
    ...
    { // build gragh
        ...
        else
        {
            ...
            if (mgr.FF_Map.count(driving_cell))
            { // drive by FF
                ...
                if (mgr.FF_Map.count(instanceName))
                { // to FF
                ...
                else if (mgr.Gate_Map.count(instanceName))
                { // to std cell
